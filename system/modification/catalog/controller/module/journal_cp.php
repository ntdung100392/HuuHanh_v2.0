...
class ControllerModuleJournalCp extends Controller {
	...
	public function search_products() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			$query = $this->db->query("SELECT * FROM product_description where match(`name`) against ('$filter_name') LIMIT 30");
					...
					'option'     => $option_data,
					'price'      => $data['price'],
					'href'       => html_entity_decode($this->url->link('product/product', '&product_id=' . $data['product_id']), ENT_QUOTES, 'UTF-8')
                            );
                             }
