      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" class="form-horizontal">
        <fieldset>
          ...
          <div class="alert alert-info" style="float:left;margin-top: 20px;">
            Below is the average price range for labpreorder service, for your convenience. Please keep in mind that labpreorder fees are determined on a case-by-case basis, but most of the time it falls within this range. This is or the labor fee only. If your system requires replacement for some parts, the fee for each part will be assessed individually.<br />
            If your shipping address is within the US, we will return your package for free, using standard shipping, after the labpreorder is done.<br /><br />
            ...
            Please bear in mind that these are only approximations and cannot be a direct reflection of the exact calculations for any individual labpreorder.
        </div>
          ...
          <div class="form-group">
            ...
            <tbody>
                <?php $image_row = 0; ?>
                <?php foreach ($labpreorder_images as $labpreorder_image) { ?>
                <tr id="labpreorder-row<?php echo $image_row; ?>">
                    <td class="left"><img src="<?php echo $labpreorder_image['thumb']; ?>" alt="" id="thumb<?php echo $image_row; ?>" /><input type="hidden" name="image[' + image_row + '][image]" value="<?php echo $labpreorder_image['image']; ?>" id="image<?php echo $image_row; ?>" /></td>
                    <td class="left"><?php echo $labpreorder_image['image']; ?></td>
                    <td class="right"><a onclick="$('#labpreorder-row<?php echo $image_row; ?>').remove();"><img src="catalog/view/theme/default/image/remove.png" alt="Remove" title="Remove"></a></td>
                </tr>
                <?php $image_row++; ?>
                <?php } ?>
            </tbody>
        ...
        <script type="text/javascript">
            ...
            $('#file_upload').uploadify({
                ...
                'uploader': '<?php echo HTTP_SERVER; ?>index.php?route=account/labpreorder/multi_upload',
                'height': 32,
                'width': 100,
                ...
                'onUploadSuccess': function (file, data, response) {
                    ...
                    if (image.status === 1) {
                        html = '  <tr id="labpreorder-row' + image_row + '">';
                        ...
                        html += '    <td class="right" style="text-align: center;vertical-align: middle;"><a onclick="$(\'#labpreorder-row' + image_row + '\').remove();"><img src="catalog/view/theme/default/image/remove.png" alt="Remove" title="Remove"></a></td>';
                        html += '  </tr>';
