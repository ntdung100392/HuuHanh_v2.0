...
class ControllerSalelabpreorder extends Controller { 
	private $error = array();
  	...
  	private function getList() {
		...
		$data['sort_order_id'] = $this->url->link('sale/labpreorder', 'token=' . $this->session->data['token'] . '&sort=r.order_id' . $url, 'SSL');
  	...
  	private function getForm() {
		...
		$data['entry_action'] = $this->language->get('entry_action');
                $data['entry_labpreorder_reason'] = $this->language->get('entry_labpreorder_reason');
    	...
    	if (isset($this->request->post['order_id'])) {
			...
			$data['order_id'] = $labpreorder_info['order_id'];
		} else {
      		$data['order_id'] = '';
    	}	
    	...
    	if (isset($this->request->post['date_ordered'])) {
			...
			$data['date_ordered'] = $labpreorder_info['date_ordered'];
		} else {
      		$data['date_ordered'] = '';
    	}	
		...
		if (isset($this->request->post['product_id'])) {
			...
			$data['product_id'] = $labpreorder_info['product_id'];
		} else {
			$data['product_id'] = '';
		}	
		...
		if (isset($this->request->post['opened'])) {
			...
			$data['opened'] = $labpreorder_info['opened'];
		} else {
			$data['opened'] = '';
		}
		
		if (isset($this->request->post['labpreorder_reason_id'])) {
			$data['labpreorder_reason_id'] = $this->request->post['labpreorder_reason_id'];
		} elseif (!empty($labpreorder_info)) {
			$data['labpreorder_reason_id'] = $labpreorder_info['labpreorder_reason_id'];
		} else {
			$data['labpreorder_reason_id'] = '';
		}
							
		$this->load->model('localisation/labpreorder_reason');
		
		$data['labpreorder_reasons'] = $this->model_localisation_labpreorder_reason->getlabpreorderReasons();
	...
	public function info() {
		...
		if ($labpreorder_info) {
			...
			$order_info = $this->model_sale_order->getOrder($labpreorder_info['order_id']);

			$data['token'] = $this->session->data['token'];
			
			$data['labpreorder_id'] = $labpreorder_info['labpreorder_id'];
			$data['order_id'] = $labpreorder_info['order_id'];
                        $data['login_id'] = $labpreorder_info['login_id'];
                        $data['shipping_address'] = $labpreorder_info['shipping_address'];
									
			if ($labpreorder_info['order_id'] && $order_info) {
				$data['order'] = $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $labpreorder_info['order_id'], 'SSL');
			} else {
				$data['order'] = '';
			}
			
			$data['date_ordered'] = date($this->language->get('date_format_short'), strtotime($labpreorder_info['date_ordered']));
			$data['firstname'] = $labpreorder_info['firstname'];
			$data['lastname'] = $labpreorder_info['lastname'];
			...
			$labpreorder_reason_info = $this->model_localisation_labpreorder_reason->getlabpreorderReason($labpreorder_info['labpreorder_reason_id']);

			if ($labpreorder_reason_info) {
				$data['labpreorder_reason'] = $labpreorder_reason_info['name'];
			} else {
				$data['labpreorder_reason'] = '';
			}			
			
			$data['opened'] = $labpreorder_info['opened'] ? $this->language->get('text_yes') : $this->language->get('text_no');
			...
			$data['labpreorder_status_id'] = $labpreorder_info['labpreorder_status_id'];                        
                        ...
                        $data['labpreorder_images'] = array();

                        foreach ($this->model_sale_labpreorder->getlabpreorderImages($labpreorder_id) as $labpreorder_image)
                        {
                            if ($labpreorder_image['image'] && file_exists(DIR_IMAGE . $labpreorder_image['image']))
                            {
                                $image = $labpreorder_image['image'];
                            }
                            else
                            {
                                $image = 'no_image.jpg';
  	...
  	private function validateForm() {
		...
		if (empty($this->request->post['labpreorder_reason_id'])) {
