...
class ModelSalelabpreorder extends Model {
        public function getlabpreorderImages($repai_id)
       {
           $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "labpreorder_image WHERE labpreorder_id = '" . (int) $repai_id . "'");

           return $query->rows;
       }
    
	public function addlabpreorder($data) {
      	$this->db->query("INSERT INTO `" . DB_PREFIX . "labpreorder` SET order_id = '" . (int)$data['order_id'] . "', product_id = '" . (int)$data['product_id'] . "', customer_id = '" . (int)$data['customer_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', product = '" . $this->db->escape($data['product']) . "', model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', opened = '" . (int)$data['opened'] . "', labpreorder_reason_id = '" . (int)$data['labpreorder_reason_id'] . "', labpreorder_action_id = '" . (int)$data['labpreorder_action_id'] . "', labpreorder_status_id = '" . (int)$data['labpreorder_status_id'] . "', comment = '" . $this->db->escape($data['comment']) . "', date_ordered = '" . $this->db->escape($data['date_ordered']) . "', date_added = NOW(), date_modified = NOW()");
	}
	
	public function editlabpreorder($labpreorder_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "labpreorder` SET order_id = '" . (int)$data['order_id'] . "', product_id = '" . (int)$data['product_id'] . "', customer_id = '" . (int)$data['customer_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', product = '" . $this->db->escape($data['product']) . "', model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', opened = '" . (int)$data['opened'] . "', labpreorder_reason_id = '" . (int)$data['labpreorder_reason_id'] . "', labpreorder_action_id = '" . (int)$data['labpreorder_action_id'] . "', labpreorder_status_id = '" . (int)$data['labpreorder_status_id'] . "', comment = '" . $this->db->escape($data['comment']) . "', date_ordered = '" . $this->db->escape($data['date_ordered']) . "', date_modified = NOW() WHERE labpreorder_id = '" . (int)$labpreorder_id . "'");
	}
	...
	public function getTotallabpreordersBylabpreorderReasonId($labpreorder_reason_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "labpreorder` WHERE labpreorder_reason_id = '" . (int)$labpreorder_reason_id . "'");
