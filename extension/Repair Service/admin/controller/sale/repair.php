	...
	private $error = array();
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/repair', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	private function getList() {
		...
		$data['add'] = $this->url->link('sale/repair/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$repair_total = $this->model_sale_repair->getTotalRepairs($filter_data);
	
		$results = $this->model_sale_repair->getRepairs($filter_data);
			...
			$data['repairs'][] = array(
				...
				'selected'      => isset($this->request->post['selected']) && in_array($result['repair_id'], $this->request->post['selected']),
                                'view'          => $this->url->link('sale/repair/info', 'token=' . $this->session->data['token'] . '&repair_id=' . $result['repair_id'] . $url, 'SSL'),
				'edit'          => $this->url->link('sale/repair/update', 'token=' . $this->session->data['token'] . '&repair_id=' . $result['repair_id'] . $url, 'SSL')
			);
		}	
		...
		$data['column_action'] = $this->language->get('column_action');
                ...
                $data['entry_repair_id'] = $this->language->get('entry_repair_id');
  	...
  	private function getForm() {
    	$data['heading_title'] = $this->language->get('heading_title');
                ...
                $data['text_form'] = !isset($this->request->get['repair_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		$data['entry_action'] = $this->language->get('entry_action');
                $data['entry_repair_reason'] = $this->language->get('entry_repair_reason');
		...
		$data['token'] = $this->session->data['token'];
			...
			$data['repair_id'] = $this->request->get['repair_id'];
		} else {
			$data['repair_id'] = 0;
		}
		...
		$this->response->setOutput($this->load->view('sale/repair_form.tpl', $data));
	}
	...
	public function info() {
		...
		if ($repair_info) {
			...
			$data['header'] = $this->load->controller('common/header');
                    ...
                    $this->response->setOutput($this->load->view('sale/repair_info.tpl', $data));	
		} else {
	...
	public function history() {
		...
		$data['pagination'] = $pagination->render();	
                ...
                $data['results'] = sprintf($this->language->get('text_pagination'), ($history_total) ? (($page - 1) * 10) + 1 : 0, ((($page - 1) * 10) > ($history_total - 10)) ? $history_total : ((($page - 1) * 10) + 10), $history_total, ceil($history_total / 10));
                    ...
                    $this->response->setOutput($this->load->view('sale/repair_history.tpl', $data));
  	}		
}
?>