...
class ControllerCheckoutCheckoutTwo extends Controller {
	...
	public function index() {
		...
		if (version_compare(VERSION, '1.5.1.3', '>') == true) {
			...
			$this->data['agree_button_html'] 			= '<input type="button" onclick="alert(\''. $this->language->get('text_must_agree') .'\'); return false;" class="button" value="'. $this->data['button_confirm'] .'" />';
			...
			$this->data['agree_button_html'] 			= '<a onclick="alert(\''. $this->language->get('text_must_agree') .'\'); return false;" class="button151"><span>'. $this->data['button_confirm'] .'</span></a>';
		} else {
			...
			$this->data['agree_button_html'] 			= '<a onclick="alert(\''. $this->language->get('text_must_agree') .'\'); return false;" class="button"><span>'. $this->data['button_confirm'] .'</span></a>';
		}
...
/////////
    	...
    	foreach ($this->cart->getProducts() as $product) {
			...
			if ($product['image']) {
				$image = $this->model_tool_image->resize($product['image'], 24, 24);
			} else {
				$image = $this->model_tool_image->resize('no_image.jpg', 24, 24);
			}
		...
		$this->data['show_image'] = (int)$this->config->get('uber_checkout_show_image');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/checkout_two.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/checkout_two.tpl';
		} else {
  	...
  	private function addOrder() {
		...
		if (isset($payment_address['company_id'])) {
			...
			$data['payment_company_id'] = 0; // v153
			$data['payment_tax_id'] = 0; // v153
		}
	...
	public function couponPaymentTotals() {
	   	if (isset($this->request->post['coupon'])) {
			...
			// Refresh the temp order 1: Load this first to update the totals
			$this->addOrder();
			...
			// Refresh the temp order 2: Load this again to update the new totals for mods like Payment Type Fee
			$this->addOrder();
			...
			//}
			// Conundrum:
			// Payment needs to load before totals for mods like Payment Type Fee
			// But Order totals load first for everything else.
			// Can't load payment twice since it doesn't use require_once
			//$json['payment'] = $this->getChild('payment/' . $this->session->data['payment_method']['code']);
