...
class ControllerCheckoutCheckoutOne extends Controller {
	...
	private $requireAccount = true;
  	...
  	public function index() {
		...
		// Buttons
		if (version_compare(VERSION, '1.5.2.1', '>') == true) {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/checkout_one.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/checkout_one.tpl';
		} else {
