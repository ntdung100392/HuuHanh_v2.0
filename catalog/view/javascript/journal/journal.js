	...
	$(function(){
		...
		$('#search input[type="text"]').live('keydown', function(e) {
			...
			if (e.keyCode == 13) {
                ...
                /*
                $('#search2 input[type="text"]').live('keydown', function(e) {
			if (e.keyCode == 13) {
				url = $('base').attr('href') + 'index.php?route=product/search';

				var filter_name = $('#search2 input[type="text"]').attr('name');
				var filter_value = $('#search2 input[type="text"]').attr('value');
			...
			$('#search input').autocompletesearch({
				...
				transformResult: function(response) {
					...
					return {
                        ...
                        $('#search2 input').autocompletesearch({
				serviceUrl: 'index.php?route=module/journal_cp/search_products',
				paramName: 'filter_name',
				onSelect: function (suggestion) {
					location = suggestion.data.href;
